...
def simulate_network_node_degree_fee_variation(num_nodes, capacity_range, transaction_amount, fee_range, epsilon, window_size, num_runs, avg_degree_range, checkpointing = False, checkpoint_interval = 20):
    """
        ...
        for degree in avg_degree:
            for run in range(num_runs):
                ...
                if checkpointing == True and run % checkpoint_interval == 0:
                    ...
                    checkpoint_filename = f'checkpoint_avg_degree_fixed_{degree}_fee_{fee}_run_{run}.pkl'
...
def plot_results_node_degree_fee_variation(df):
    """
    ...
    sns.lineplot(data=df, x='avg_degree', y='success_rate', hue='fee', marker='o', ci='sd', legend='full')
    ...
    plt.tight_layout()
    fig.savefig('node_degree_vs_fees_vm_capacity_7_finer_20.png', dpi=300, bbox_inches='tight')
    plt.show()
...
fee_range = [0.1, 0.125, 0.2, 0.25, 0.4, 0.5, 0.6, 0.75, 0.8, 1]
...
# df = pd.read_pickle('node_degree_capacity_5_finer_variation.pkl')
...
df.to_pickle('node_degree_capacity_7_finer_20_variation.pkl')
#
# # Plotting
