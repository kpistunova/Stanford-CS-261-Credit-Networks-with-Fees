...
palette = sns.color_palette("coolwarm", n_colors=df['nodes'].nunique())

for fee in df['fee'].unique():
    if fee != 0.0:
        # x is capacity
        df_filtered = df[(df['fee'].isin([fee, 0.0]))]
        fig, ax = plt.subplots(figsize=(8 , 6 / 1.2), dpi=300)

        # Use 'fee' as the style variable
        sns.lineplot(data=df_filtered, x='capacity', y='avg_path_length', hue='nodes', style='fee', dashes=True, markers = True,
                     palette=palette, hue_norm=matplotlib.colors.LogNorm(), ci='sd', alpha = 0.8)
        title = graph_type + ' graph'
        plt.title(title, fontsize=16)
